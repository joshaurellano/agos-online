"""
Trains ONLY the CNN (Conv1D) encoder-decoder flood-forecast model.
...
Writes:
    flood_cnn_14day_encdec_model.h5    <- the model this API serves
    CNN_per_horizon_metrics.csv        <- per-day accuracy/precision/recall/F1
    CNN_horizon_skill_chart.png
    CNN_confusion_matrix_day1_vs_day14.png
...
if __name__ == "__main__":
    ...
    print("\n>>> Training CNN (encoder-decoder, 14-day, w/ forecast input)...")
    model = build_encdec("cnn", data["past_shape"], data["future_shape"], HORIZON, data["loss_fn"])
    model.fit(
    ...
    evaluate_multihorizon(
        model, [data["X_past_test"], data["X_future_test"]], data["y_test"], "CNN"
    )

    model.save("flood_cnn_14day_encdec_model.h5")
    print("-> Saved: flood_cnn_14day_encdec_model.h5")
