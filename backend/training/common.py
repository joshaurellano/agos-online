"""
Shared training logic used by train_lstm.py / train_gru.py / train_cnn.py.

This used to all live in one file (train_models.py, still here for
reference/backward-compat). It's split now so each algorithm can be
trained, tuned, and re-run independently -- but they all go through the
exact same dataset loading, feature engineering, sequence-building, and
encoder-decoder architecture, so the three .h5 files stay fairly
comparable and match what app/features/engineering.py expects at
serve time.

IMPORTANT: run the train_*.py scripts from the PROJECT ROOT, e.g.:

    python training/train_lstm.py

All file paths (dataset CSVs in, .h5/.pkl/.json/.csv out) are relative
to the current working directory, not to this file's location -- the
serving app (app/config/settings.py) expects those same filenames to
...
#
# Put YOUR collected dataset here as one of these two filenames
# (backfilled/enriched one takes priority if both exist). It must sit
# at the project root, next to main.py -- not inside training/.
# ────────────────────────────────────────────────────────────────
DATA_FILE_CANDIDATES = ["flood_ml_dataset_backfilled.csv", "flood_ml_dataset_clean.csv"]

# Columns that only exist in the Open-Meteo-backfilled/enriched dataset.
# When present, preprocess_flood_data() automatically adds them as extra
# model features -- no code change needed, it's detected from the CSV.
...
def load_flood_dataset(candidates=DATA_FILE_CANDIDATES):
    """
    Tries each candidate CSV in order (backfilled first) and loads the
    first one that exists on disk. Returns (df, path_used, used_backfilled).
    """
    ...
    raise FileNotFoundError(
        f"None of these CSVs were found in your working directory: {candidates}. "
        "Put your collected dataset at the project root using one of these "
        "filenames (run the script with `python training/train_<algo>.py` "
        "from AI_Model/, not from inside training/)."
    )


# ────────────────────────────────────────────────────────────────
# STEP 1: FEATURE ENGINEERING (identical for all 3 algorithms -- this
#   is what keeps GRU/LSTM/CNN directly comparable and matches
#   app/features/engineering.py at serve time)
# ────────────────────────────────────────────────────────────────
def preprocess_flood_data(df):
    ...
    if col_rain is None or col_target is None or col_month is None:
        raise ValueError(
            "Your dataset must have at minimum a month column, a rainfall "
            "column (name containing 'rainfall'), and a flood outcome "
            "column (name containing 'flood'). Found columns: "
            f"{list(df.columns)}"
        )
    ...
    # Enriched Open-Meteo columns -- only present when trained on an
    # enriched/backfilled dataset. Added by exact name since these are
    # already unambiguous, fixed column names.
...
# ────────────────────────────────────────────────────────────────
# STEP 3: WEIGHTED BINARY CROSSENTROPY
# ────────────────────────────────────────────────────────────────
...
# ────────────────────────────────────────────────────────────────
# STEP 5: PER-HORIZON-DAY EVALUATION
# ────────────────────────────────────────────────────────────────
def evaluate_multihorizon(model, X_test, y_test, name, horizon=HORIZON):
    ...
    try:
        ...
        print(f"Warning: could not save horizon skill chart for {name}: {e}")
    ...
    try:
        ...
        print(f"Warning: could not save confusion matrix plot for {name}: {e}")

    return result_df


# ────────────────────────────────────────────────────────────────
# SHARED PIPELINE: dataset -> scaled sequences -> train/test split.
#
# Every train_<algo>.py script calls this FIRST. It is 100%
# deterministic given the same CSV (fixed SEED, no shuffling before
# the split), so it's safe to call independently from three separate
# script runs -- flood_scaler.pkl and feature_metadata.json will come
# out byte-for-byte the same regardless of which script wrote them
# last, as long as they were all trained on the same dataset file.
# ────────────────────────────────────────────────────────────────
def prepare_training_data():
    df, csv_path, used_backfilled = load_flood_dataset()
    print(f"Dataset loaded: {csv_path}"
          + (" (ENRICHED/backfilled data)" if used_backfilled
             else " (base dataset -- enriched/backfilled CSV not found, fell back)"))
    ...
    if enriched_used:
        print(f"  -> includes {len(enriched_used)} enriched feature(s): {enriched_used}")
    ...
    pos_weight = 3.5  # tuned value; see train_models.py comments for the sweep that picked this
    print(f"Flood rate in training set: {flood_rate:.3f}")
    print(f"  auto-computed pos_weight would have been: {auto_pos_weight:.2f} (too aggressive)")
    print(f"  pos_weight used (tuned): {pos_weight:.2f}")
    ...
    _write_feature_metadata(
        features=features, future_features=FUTURE_FEATURES, target_col=target_col,
        flood_rate=flood_rate, pos_weight=pos_weight, csv_path=csv_path,
        used_backfilled=used_backfilled, enriched_used=enriched_used,
        col_moisture=col_moisture, col_signal=col_signal, df_processed=df_processed,
    )
    ...
    return {
        ...
        "past_shape": past_shape, "future_shape": future_shape,
        "loss_fn": loss_fn,
    }


def _write_feature_metadata(features, future_features, target_col, flood_rate,
                             pos_weight, csv_path, used_backfilled, enriched_used,
                             col_moisture, col_signal, df_processed):
    """
    Writes feature_metadata.json with everything EXCEPT the top-level
    "reliability" block, which depends on a trained model's own metrics
    and gets filled in by merge_metrics.py (or by train_gru.py directly,
    since gru is FLOOD_DEFAULT_MODEL) after training finishes. If a
    previous run already wrote a real reliability block, it's preserved
    here instead of getting reset to the fallback.
    """
    existing_reliability = None
    if os.path.exists("feature_metadata.json"):
        try:
            with open("feature_metadata.json", "r") as f:
                existing_reliability = json.load(f).get("reliability")
        except Exception:
            pass
    ...
    metadata = {
        ...
        "future_feature_order": future_features,
        ...
        "note": ("typhoon_signal in the dataset is 0-4 (same scale as the "
                 "wind_to_signal() thresholds in the API backend, so no "
                 "remapping is needed). prev_flood is EXCLUDED from "
                 "future_feature_order because it isn't known in advance. "
                 "enriched_features_used (if any) must also be fetched by "
                 "app/features/engineering.py from the live Open-Meteo "
                 "call -- that's what links the training dataset to the "
                 "live inference features."),
        "reliability": existing_reliability or {
            "measured_on": "not yet computed -- train at least one model",
            "avg_precision_across_14day_horizon": None,
            "avg_recall_across_14day_horizon": None,
            "avg_f1_across_14day_horizon": None,
            "avg_false_alarm_rate": None,
        },
    }
...
def set_reliability_from_metrics(result_df):
    """
    Overwrites feature_metadata.json's top-level "reliability" block
    using this model's own per-horizon metrics. Called by train_gru.py
    (gru is FLOOD_DEFAULT_MODEL in app/config/settings.py) so the
    default model's numbers are what the legacy /api/predict-flood and
    /api/forecast-flood endpoints report. Safe to call from any script,
    or from merge_metrics.py, if you change the default model.
    """
    with open("feature_metadata.json", "r") as f:
    ...
    metadata["reliability"] = {
        ...
        "avg_precision_across_14day_horizon": float(result_df['precision'].mean()),
        "avg_recall_across_14day_horizon": float(result_df['recall'].mean()),
        "avg_f1_across_14day_horizon": float(result_df['f1_score'].mean()),
        "avg_false_alarm_rate": float(result_df['false_alarm_rate'].mean()),
        "honest_caveat": (
            "Metrics beyond ~day 7-10 also inherit Open-Meteo's own "
            "forecast uncertainty at longer lead times, on top of the "
            "model's own error -- treat flood_probability as "
