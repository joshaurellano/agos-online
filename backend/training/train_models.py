"""
Training script for the flood-forecast models.

Trains THREE encoder-decoder architectures (LSTM, GRU, CNN) on the same
data/features/horizon and saves all three, plus the shared scaler and
feature_metadata.json that app/models/registry.py loads at serve time.

IMPORTANT: run this from the PROJECT ROOT, e.g.:

    python training/train_models.py

All file paths below (dataset CSVs in, .h5/.pkl/.json/.csv out) are
relative to the current working directory, not to this file's location
-- the serving app (app/config/settings.py) expects those same
filenames to exist at the project root.
"""
